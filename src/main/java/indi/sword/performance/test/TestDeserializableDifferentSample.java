 ...
 * @MODIFIED BY
 */
public class TestDeserializableDifferentSample {
    ...
    private static void singleThread(int sampleSize, int listSize, int mapKeyNum) throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            List<String> jsonList = SampleBuilder.buildJsonSamples(sampleEntityList,type);
            long cost = 0;
            try {
            ...
            String outputPath = FileUtil.getDeserializableOutputPath(type, sampleSize, listSize, mapKeyNum,
                    1) ;
